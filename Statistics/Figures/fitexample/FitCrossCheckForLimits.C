...
stored in a the rootfile FitCrossChecks.root. Please use root version >=5.34.04. 
...
namespace LimitCrossCheck{  
  ...
  bool plotSubchannels=true;               // create postfit plots which are stored in FitCrossCheck.root
  ...
  RooFitResult* FitPDF( ModelConfig* model, RooAbsPdf* fitpdf, RooAbsData* fitdata, 
    ...
    while ((nui_arg = (RooAbsArg*)nIter->Next())){
      ...
      TString nuiname = thisNui->GetName();
      if (nuiname.Contains("TRF_factor")){
        std::cout << nuiname << std::endl;
        thisNui->setConstant(true);
        w->var(nuiname)->setConstant(true);
      }
    }

    int numCPU = sysconf( _SC_NPROCESSORS_ONLN ); 
    //RooAbsReal * nll = fitpdf->createNLL(*fitdata, Constrain(*constrainedParams), GlobalObservables(*glbObs), NumCPU(numCPU, RooFit::Hybrid));
    RooAbsReal * nll = fitpdf->createNLL(*fitdata, Constrain(*constrainedParams), GlobalObservables(*glbObs), Offset(1), NumCPU(numCPU, RooFit::Hybrid));
  ...
  void PlotHistosAfterFitEachSubChannel(bool IsConditionnal, double mu){
    ...
    while((tt=(RooCatType*) iter->Next()) ){
      ...
      TString fnuisfcorr = OutputDir + "/TextFileFitResult/Fit"+(TString)tt->GetName()+"_fitresFull_"+TS_IsConditionnal+"_mu";
      fnuisfcorr += mu;
      fnuisfcorr += ".txt";
      ofstream fnuisParAndFullCorr(fnuisfcorr.Data());
      TString fnuisfcorrNoG = OutputDir + "/TextFileFitResult/Fit"+(TString)tt->GetName()+"_fitresNoG_"+TS_IsConditionnal+"_mu";
      fnuisfcorrNoG += mu;
      fnuisfcorrNoG += ".txt";
      ofstream fnuisParAndCorrNoG(fnuisfcorrNoG.Data());
      fnuisParAndCorr << "NUISANCE_PARAMETERS" << endl;
      fnuisParAndFullCorr << "NUISANCE_PARAMETERS" << endl;
      fnuisParAndCorrNoG << "NUISANCE_PARAMETERS" << endl;
      ...
      if(!IsConditionnal){
        ...
        fnuisParAndCorr << "SigXsecOverSM   " << firstPOI->getVal() << "  + " << firstPOI->getErrorHi() << "  - " << fabs(firstPOI->getErrorLo()) << endl; 
        fnuisParAndFullCorr << "SigXsecOverSM   " << firstPOI->getVal() << "  + " << firstPOI->getErrorHi() << "  - " << fabs(firstPOI->getErrorLo()) << endl; 
        fnuisParAndCorrNoG << "SigXsecOverSM   " << firstPOI->getVal() << "  + " << firstPOI->getErrorHi() << "  - " << fabs(firstPOI->getErrorLo()) << endl; 
      ...
      while( (var = (RooRealVar*) it2->Next()) ){
        ...
        fnuisParAndFullCorr << var->GetName() << " " << pull << "  " << fabs(errorHi) << "  -" << fabs(errorLo) << endl;

        if ( (varname.find("gamma_stat")!=string::npos) ) continue;

        fnuisParAndCorrNoG << var->GetName() << " " << pull << "  " << fabs(errorHi) << "  -" << fabs(errorLo) << endl;
      ...
      fnuisParAndFullCorr << endl << endl << "CORRELATION_MATRIX" << endl;
      fnuisParAndCorrNoG << endl << endl << "CORRELATION_MATRIX" << endl;

      fnuisParAndCorr << h2Dcorrelation->GetNbinsX() << "   " << h2Dcorrelation->GetNbinsY() << endl;
      fnuisParAndCorrNoG << h2Dcorrelation->GetNbinsX() << "   " << h2Dcorrelation->GetNbinsY() << endl;
      // for(int kk=1; kk < h2Dcorrelation->GetNbinsX()+1; kk++) {
      for(int kk=1; kk < ibNPs+1; kk++) {
        // for(int ll=1; ll < h2Dcorrelation->GetNbinsY()+1; ll++) {
        for(int ll=h2Dcorrelation->GetNbinsY(); ll > h2Dcorrelation->GetNbinsY()-ibNPs; ll--) {
          fnuisParAndCorr << h2Dcorrelation->GetBinContent(kk,ll) << "   ";
          fnuisParAndCorrNoG << h2Dcorrelation->GetBinContent(kk,ll) << "   ";
        }
        fnuisParAndCorr << endl;
        fnuisParAndCorrNoG << endl;
      }
      fnuisParAndCorr << endl;
      fnuisParAndCorr.close();
      fnuisParAndCorrNoG << endl;
      fnuisParAndCorrNoG.close();

      fnuisParAndFullCorr << h2Dcorrelation->GetNbinsX() << "   " << h2Dcorrelation->GetNbinsY() << endl;
      for(int kk=1; kk < h2Dcorrelation->GetNbinsX()+1; kk++) {
        for(int ll=1; ll < h2Dcorrelation->GetNbinsY()+1; ll++) {
          fnuisParAndFullCorr << h2Dcorrelation->GetBinContent(kk,ll) << "   ";
        }
        fnuisParAndFullCorr << endl;
      }
      fnuisParAndFullCorr << endl;
      fnuisParAndFullCorr.close();
      ...
      gROOT->cd();
      //RooAbsReal* nll = pdftmp->createNLL(*datatmp, Offset(1));
      RooAbsReal* nll = pdftmp->createNLL(*datatmp, Offset(1), NumCPU(10, RooFit::Hybrid));
      if (!IsConditionnal){
    ...
    void PlotHistosAfterFitGlobal(bool IsConditionnal, double mu){
      ...
      //FIXME needed? if(!IsConditionnal) NparNotStat++;
      ...
      fnuiscorr += ".txt";
      ofstream fnuisParAndCorr(fnuiscorr.Data());
      TString fnuisfcorr = OutputDir + "/TextFileFitResult/GlobalFit_fitresFull_"+TS_IsConditionnal+"_mu";
      fnuisfcorr += mu;
      fnuisfcorr += ".txt";
      ofstream fnuisParAndFullCorr(fnuisfcorr.Data());
      TString fnuisfcorrNoG = OutputDir + "/TextFileFitResult/GlobalFit_fitresNoG_"+TS_IsConditionnal+"_mu";
      fnuisfcorrNoG += mu;
      fnuisfcorrNoG += ".txt";
      ofstream fnuisParAndCorrNoG(fnuisfcorrNoG.Data());
      fnuisParAndCorr << "NUISANCE_PARAMETERS" << endl;
      ...
      fnuisPar << "EDM = " << fitresGlobal->edm() << endl;
      fnuisPar << "minNll = " << fitresGlobal->minNll() << endl;
      fnuisPar << "status = " << fitresGlobal->status() << endl;
      ...
      while( (var = (RooRealVar*) it2->Next()) ){
        ...
        vname.ReplaceAll("alpha_","");
        vname.ReplaceAll("Lumi","Luminosity");
        TString vnametxt = vname;
        vname.ReplaceAll("_","\\_");
        fnuisPar.precision(3);
        fnuisPar << vname << " & $" << pull << "^{+" << fabs(errorHi) << "}_{-" << fabs(errorLo)  << "}$ \\\\" << endl;
        fnuisParAndCorr << vnametxt << "   " << pull << "  " << fabs(errorHi) << "  -" << fabs(errorLo)  << endl;
      ...
      fnuisParAndCorr << endl << endl << "CORRELATION_MATRIX" << endl;
      fnuisParAndCorr << NparNotStat << "   " << NparNotStat << endl;
      fnuisParAndCorrNoG << endl << endl << "CORRELATION_MATRIX" << endl;
      fnuisParAndCorrNoG << NparNotStat << "   " << NparNotStat << endl;
      for(int kk=1; kk < h2Dcorrelation->GetNbinsX()+1; kk++) {
        TString lab = h2Dcorrelation->GetXaxis()->GetBinLabel(kk);
        if(lab.Contains("ttH-Scale") && IsConditionnal) continue;
        if(lab.Contains("gamma")) continue;
        // cout << "[35mLabel = " << h2Dcorrelation->GetXaxis()->GetBinLabel(kk) << "  N params = " << ib << "[0m" << endl;
        for(int ll=h2Dcorrelation->GetNbinsY(); ll > 0; ll--) {
          TString lab2 = h2Dcorrelation->GetYaxis()->GetBinLabel(ll);
          if(lab2.Contains("ttH-Scale") && IsConditionnal) continue;
          if(lab2.Contains("gamma")) continue;
          // cout << "[32mLabel = " << h2Dcorrelation->GetYaxis()->GetBinLabel(ll) << " = " << h2Dcorrelation->GetBinContent(kk,ll) << "    [0m";
          fnuisParAndCorr << h2Dcorrelation->GetBinContent(kk,ll) << "   ";
          fnuisParAndCorrNoG << h2Dcorrelation->GetBinContent(kk,ll) << "   ";
        }
        fnuisParAndCorr << endl;
      ...
      if(draw1DResponse) {
        ...
        gROOT->cd();
        RooAbsReal* nll = simPdf->createNLL(*data, Constrain(*mc->GetNuisanceParameters()), 
            //GlobalObservables(*globObs), 
            Offset(1),
            NumCPU(10, RooFit::Hybrid));
        ...
        */
        ...
        //Plot1DResponse(nll, firstPOI, "", can2, poly2, true, latex, NLLprojection);
        ...
        while( (var = (RooRealVar*) it3->Next()) ){
          ...
          if(!vname.Contains("ttbar_bb")) continue;
          cout << " --> NP: " << vname << endl;
      ...
      //      2D CONTOUR PLOT BETWEEN CHOSEN PARAMETERS
      //
      //      if (!IsConditionnal){
      //        gROOT->cd();
      //        outputfile->cd();
      //        TDirectory *NLL2dfolder = (TDirectory*)MainDir->mkdir("NLL2Dproj");
      //        gROOT->cd();
      //
      //        RooAbsReal* nll3 = simPdf->createNLL(*data,RooFit::Extended(kTRUE));
      //        nll3->enableOffsetting(1);
      //        RooMinuit minuit(*nll3) ;
      //        minuit.migrad() ;
      //        minuit.hesse() ;
      //        RooRealVar* varMu= (RooRealVar*) firstPOI;
      //
      //        RooRealVar* varOther;
      //        TIterator* it3 = mc->GetNuisanceParameters()->createIterator();
      //        while( (varOther = (RooRealVar*) it3->Next()) ){
      //          TString vname=varOther->GetName();
      //          if (vname.Contains("HFfrac") || vname.Contains("HF-SF") ) break;
      //        }
      //        TCanvas* c2D = new TCanvas( "mu_VS_HFfrac" );
      //        //varMu  ->enableOffsetting(1);                                                                                                                                                                     
      //        //varOther->enableOffsetting(1);                                                                                                                                                                    
      //        RooPlot* pC1 = minuit.contour(*varOther, *varMu,1,2) ; // 1 and 2 sigma                                                                                                                             
      //        pC1->Draw() ;
      //        NLL2dfolder->cd();
      //        c2D->Write();
      //      }
    ...
    void Plot1DResponse(RooAbsReal* nll, RooRealVar* var, TString cname, TCanvas* can, TF1* poly, bool IsFloating, TLatex* latex, TDirectory* tdir, RooArgSet* SliceSet) {
      ...
      // this is Wouter's way.
      RooAbsReal* pll = nll->createProfile(*var);//,NumCPU(10),Offset(1)) ;
      RooPlot* frame2 = var->frame(Title("-log(L) vs "+vname+" "+cname),Bins(10),Range(-1.5,1.5));
      pll->plotOn(frame2,RooFit::Precision(-1),LineColor(kRed), NumCPU(3));
      RooCurve* curve = frame2->getCurve();
      ...
      if(!IsFloating) {
        ...
        frame2->GetXaxis()->SetRangeUser(-1.5,1.5);
        curve = poissonTerm;
      } else {
    ...
    bool IsAnormFactor(RooRealVar *var){
      ...
      string varname = (string) var->GetName();
      const double val =  MapNuisanceParamNom[varname];
      if (((TString)varname).Contains("TRF") || (!((TString)varname).Contains("_stat_") && val==1.0)) result=true;
    ...
    void Initialize(const char* infile , const char* outputdir, const char* workspaceName, const char* modelConfigName, const char* ObsDataName) {
      ...
      if(!data){
        std::cout << "Couldn't find data! " << ObsDataName << std::endl;
        getchar();
      } 
      std::cout << TString(infile) <<std::endl;          
      if(TString(infile).Contains("combWS")){
        w->var("alpha_ttbar_PartonShower-cc")->setVal(0.9);
        w->var("alpha_ttbar-DataRw-IFSR")->setVal(-0.8);
        w->var("alpha_ttbar_bb")->setVal(0.5);
        w->var("alpha_JVF")->setVal(-0.2);
      }
    ...
    void unfoldConstraints(RooArgSet& initial, RooArgSet& final, RooArgSet& obs, RooArgSet& nuis, int& counter)
    {
      ...
      RooDataSet* makeAsimovData(double mu_val, bool fluctuateData, string* mu_str){
        ...
        for (int i=0;i<nrNuis;i++){
          ...
          //if( TString(glob->GetName()).Contains("MEM_D1")){ //FIXME
          //  glob->setVal(0.9999);
          //  nui->setVal(0.9999);
          //  nui->setConstant();
          //}
