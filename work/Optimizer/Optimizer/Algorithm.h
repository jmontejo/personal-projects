...
class Algorithm {
  ...
  protected:
    Algorithm(VariablePool *vars);
		...
		bool checkMetric(float val, float sig, float bkg, float bkg_effev, std::vector<double> max_pars, const Double_t *par = NULL);
		...
		void getSB(const Double_t *par, Double_t *sig_integral, Double_t *bkg_integral, Double_t *bkg_effev=NULL, bool do_displayweight=false);
